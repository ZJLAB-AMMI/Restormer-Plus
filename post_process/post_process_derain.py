...
est_pixels_file = 'D:/job/competition/gt_rain_best_result/saved_pixels_all.pkl'
est_pixels = pickle.load(open(est_pixels_file, 'rb'))

print(est_pixels)
print(len(est_pixels))
print(len(est_pixels['123_0']))

new_est_pixels = {}
for scene_name, pixel_data in est_pixels.items():
    new_est_pixels[scene_name] = []
    for pix in pixel_data:
        new_est_pixels[scene_name].append(
            pix['pos']
        )

fw = open('D:/job/competition/gt_rain_best_result/pixels.pkl', 'wb')
pickle.dump(new_est_pixels, fw)
print(new_est_pixels)
